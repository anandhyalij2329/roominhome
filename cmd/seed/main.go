...
func main() {
	...
	ownerHash, _ := auth.HashPassword("owner123")
	seekerHash, _ := auth.HashPassword("seeker123")
	now := time.Now().UTC()
	ownerID := "seed-owner-1"
	seekerID := "seed-seeker-1"

	_, _ = db.Exec(`DELETE FROM bookings`)
	_, _ = db.Exec(`DELETE FROM media`)
	_, _ = db.Exec(`DELETE FROM properties`)
	_, _ = db.Exec(`DELETE FROM users WHERE id LIKE 'seed-%' OR email IN (?, ?)`,
		"owner@roominhome.test", "seeker@roominhome.test")

	must(db, `INSERT INTO users (id, name, email, password_hash, role, phone, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		ownerID, "Suresh Patil", "owner@roominhome.test", ownerHash, "owner", "9876543210", now)
	must(db, `INSERT INTO users (id, name, email, password_hash, role, phone, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		seekerID, "Ananya Deshmukh", "seeker@roominhome.test", seekerHash, "seeker", "9123456780", now)
...
func must(db *sql.DB, query string, args ...any) {
